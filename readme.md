# برومبت: برنامج تجميع كونفيجات البروكسي/VPN (واجهة HTML/CSS خفيفة)

أريد منك إنشاء برنامج سطح مكتب خفيف لتجميع وإدارة كونفيجات البروكسي/VPN من مصادر متعددة، بواجهة مبنية بـ HTML/CSS/JS (مش Qt أو Tkinter)، تعمل جوه تطبيق سطح مكتب بسيط.

## الهدف العام

برنامج يقوم بـ:

1. جلب الكونفيجات من روابط GitHub (raw files) أو أي رابط اشتراك (subscription link) على الإنترنت.
2. قراءة مصادر إضافية من ملف نصي محلي (sources.txt) يحتوي على قائمة روابط، سطر لكل رابط.
3. تحليل (Parse) وفرز الكونفيجات حسب نوع البروتوكول تلقائيًا.
4. عرضها في واجهة HTML/CSS نظيفة وخفيفة مع بحث وفلترة وتصدير.

## البروتوكولات المطلوب دعمها
...
- **Backend:** Python (لسهولة التعامل مع base64 / regex / networking / async).
- **Frontend:** HTML + CSS + JavaScript خفيف (Vanilla JS، بدون فريمووركات ثقيلة).
- **الربط بين الاتنين:** pywebview (نافذة سطح مكتب محلية بدون ثقل Electron).
- تجنب Electron نهائيًا لثقله.

## المتطلبات الوظيفية

### 1. جلب البيانات (Fetching)

- حقل إدخال رابط مباشر (raw.githubusercontent.com أو أي رابط عام) وجلب محتواه.
- دعم روابط الاشتراك (subscription) المشفرة بـ base64 — decode تلقائي.
- دعم إضافة أكثر من مصدر مرة واحدة (Textarea لقائمة روابط).
- زر "Import from file" لاختيار ملف نصي محلي (كل سطر = رابط أو كونفيج مباشر).
- تحديث دوري اختياري (Auto-refresh) بفاصل زمني يحدده المستخدم.
- معالجة الأخطاء بشكل واضح في الواجهة (Toast/Notification) بدون تجميد الصفحة.

### 2. التحليل والفرز (Parsing & Classification)

- التعرف التلقائي على نوع البروتوكول من الـ URI scheme أو بنية النص.
- استخراج الحقول المهمة لكل بروتوكول (server, port, id/password, sni, alpn, security, network type... إلخ).
- إزالة التكرار (Deduplication).
- تجميع الكونفيجات وإرسالها للواجهة كـ JSON.

### 3. الواجهة (HTML/CSS/JS)

- تصميم بسيط، عصري، Dark mode افتراضي (مع خيار تبديل Light/Dark).
- شريط جانبي فيه فلاتر البروتوكولات بعدد الكونفيجات جنب كل بروتوكول.
- جدول رئيسي يعرض: الاسم/الريمارك، البروتوكول (Badge ملون)، السيرفر، البورت، الحالة.
- خانة بحث Live filtering بـ JavaScript بدون إعادة تحميل.
- زر نسخ سريع لكل كونفيج (Copy to clipboard).
- Modal/Panel جانبي لعرض تفاصيل الكونفيج كاملة.
- (اختياري) عرض حالة الاتصال Ping/Latency بلون، محدثة async من الـ backend.
- CSS عادي أو Tailwind خفيف — بدون مكتبات JS ثقيلة (Alpine.js كحد أقصى لو لزم).

### 4. التخزين والتصدير

- حفظ القوائم محليًا (SQLite أو JSON file).
- زر Export: نسخ كنص، حفظ كملف، أو تصدير base64 subscription.
- زر لدمج كل الكونفيجات في ملف واحد شامل وتصديره.

## هيكلة المشروع المطلوبة

```
project/
├── main.py              # نقطة تشغيل pywebview + ربط الـ API
├── backend/
│   ├── fetcher.py        # جلب المصادر (requests/aiohttp)
│   ├── parsers.py         # بارسر لكل بروتوكول
│   ├── storage.py         # SQLite/JSON
│   └── api.py             # الدوال المكشوفة لـ JS (js_api class)
├── frontend/
│   ├── index.html
│   ├── style.css
│   └── app.js
...
- كل بارسر بروتوكول في دالة منفصلة وواضحة عشان يسهل إضافة بروتوكولات جديدة لاحقًا.
- استخدام async/threading في جلب المصادر حتى ما تتجمدش الواجهة.
- التواصل بين JS والـ Python يكون عبر `window.pywebview.api` (الطريقة القياسية في pywebview).

## مخرجات مطلوبة

1. الكود الكامل مقسم على الملفات المذكورة فوق.
2. requirements.txt وتعليمات التشغيل خطوة بخطوة.
3. طريقة تحويله لملف تنفيذي واحد (exe) باستخدام PyInstaller مع أخذ ملفات frontend/ في الاعتبار (`--add-data`).

---

# الملحق: مواصفات تغطية بروتوكولات ConfigCrawler (الوثيقة الأساسية للمجال)

> هذه المواصفة مستوردة من `ConfigCrawler_Protocol_Coverage_Spec.md` وهي جزء من الوثيقة الأساسية للمشروع.
> أي تطوير مستقبلي في مجال الكونفيغات يجب أن يلتزم بالأهداف والمبادئ أدناه.
...
يجب أن يكون Payload عنصرًا مستقلًا في نموذج البيانات.
