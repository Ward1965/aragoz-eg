...
def _build_welcome_html():
    ...
    logo = ""
    for candidate in (os.path.join(base, "aragoz.png"), os.path.join(os.getcwd(), "aragoz.png")):
        if os.path.exists(candidate):
            try:
                ...
                continue
    logo_img = (
        f'<img id="welcomeLogo" src="data:image/png;base64,{logo}" alt="Aragoz Lite">'
        if logo
        else '<div id="welcomeLogo" style="width:88px;height:88px;border-radius:20px;background:#6366f1;color:#fff;font-size:40px;font-weight:700;display:inline-flex;align-items:center;justify-content:center;">A</div>'
    )
...
<style>
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  html, body {{ height:100%; overflow:hidden; background:transparent; font-family:'Segoe UI',Tahoma,Arial,sans-serif; }}
  .card {{ position:relative; width:calc(100% - 6px); height:calc(100% - 6px); margin:3px;
...
<body>
  <div class="card">
      <div class="hero">
        <div class="logo-wrap"><span class="glow"></span>{logo_img}</div>
